...
class FileProcessor:
    ...
    def needs_pdf_splitting(self, filename: str) -> bool:
        ...
        if not filename.lower().endswith('.pdf'):
            return False
        max_pdf_size = self.cfg.doc_processing.get('max_pdf_size', None)
        if max_pdf_size is None:
            return False
        max_pdf_size = self._parse_max_pdf_size(max_pdf_size)
        ...
        if self.contextual_chunking:
            ...
            parser = DoclingDocumentParser(
                **base_dict,
                ...
                pdf_batch_size=self.cfg.doc_processing.get('pdf_batch_size', 300),
            )
        else:
            parser = UnstructuredDocumentParser(
                **base_dict,
    ...
    def generate_vec_tables(self, tables):
        ...
        for df, summary, table_title, table_metadata in tables:
            # Validate summary before processing table
            if not summary or (isinstance(summary, str) and summary.strip() == ''):
                logger.warning(f"Skipping table '{table_title}' due to empty or missing summary")
                del df
                continue
