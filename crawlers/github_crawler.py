...
class Github(object):
    ...
    def get_issues(self, state: str) -> List[Any]:
        ...
        if response.status_code == 200:
            ...
            logger.warning(f"Error retrieving issues: {response.status_code}, {response.text}")
            return []
    ...
    def get_issue_comments(self, issue_number: str) -> List[Any]:
        ...
        if response.status_code == 200:
            ...
            logger.warning(f"Error retrieving comments: {response.status_code}, {response.text}")
            return []
    ...
    def get_pull_requests(self, state: str) -> List[Any]:
        ...
        if response.status_code == 200:
            ...
            logger.warning(f"Error retrieving pull requests: {response.status_code}, {response.text}")
            return []
    ...
    def get_pr_comments(self, pull_number: int) -> List[Any]:
        ...
        if response.status_code == 200:
            ...
            logger.warning(f"Error retrieving comments for pull request #{pull_number}: {response.status_code}, {response.text}")
            return []
            ...
            if item["type"] == "file":
                ...
                if url.lower().endswith(".md") or url.lower().endswith(".mdx"):     # Only index markdown files from the code, not the code itself
                    try:
                        ...
                        logger.error(f"Failed to retrieve content for {fname} with url {url}: {e}")
                        continue
    ...
    def crawl_repo(self, repo: str, owner: str, token: str) -> None:
        ...
        for d_issue in issues:
            ...
            try:
                ...
                logger.error(f"Error {e} indexing repo {repo}, comment document {issue_doc}")
                continue
