...
class PmcCrawler(Crawler):
    ...
    def index_papers_by_topic(self, topic: str, n_papers: int) -> None:
        ...
        for i, pmc_id in enumerate(papers):
            ...
            if pub_date is not None:
                ...
                try:
                    if isinstance(year, Tag) and isinstance(month, Tag) and isinstance(day, Tag):
                        pub_date = f"{year.text}-{month.text}-{day.text}"  # type: ignore
                    else:
                        pub_date = 'unknown'  # type: ignore
                except Exception as e:
                    pub_date = 'unknown'   # type: ignore
    
            else:
                pub_date = "Publication date not found"   # type: ignore
    ...
    def _get_xml_dict(self) -> Dict[str, Any]:
        days_back = 1
        max_days = 30
