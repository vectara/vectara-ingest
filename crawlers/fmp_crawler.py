...
class FmpCrawler(Crawler):
    ...
    def index_doc(self, document: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            logger.error(f"Error during indexing of {document['id']}: {e}")
            return False
        ...
        for year in range(self.start_year, self.end_year+1):
            for quarter in range(1, 5):
                ...
                try:
                    ...
                    logger.error(f"Error getting transcript for {company_name} quarter {quarter} of {year}: {e}")
                    continue
                if response.status_code == 200:
    ...
    def crawl(self) -> None:
        ...
        for ticker in self.tickers:
            ...
            if response.status_code == 200:
                ...
                logger.warning(f"Can't get company profile for {ticker} - skipping")
                continue
