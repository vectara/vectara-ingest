...
class HackernewsCrawler(Crawler):
    ...
    def get_comments(self, story: dict) -> List[str]:
        ...
        for kid in kids:
            try:
                ...
                logger.error(f"Error retrieving comment {kid}, e={e}")
                comment = None
    ...
    def index_story(self, id: str) -> None:
        ...
        if len(times)>0 and max(times) < datetime.datetime.now().date() - datetime.timedelta(days=self.days_back):
            logger.info(f"Skipping story {id} from date {story_date} because most recent comment is older than {self.days_back} days")
            return

        if len(texts) == 0:
            if self.verbose:
                logger.info(f"Skipping story {id} from date {story_date} because it has no text")
            return
    ...
    def fetch_stories_before_n_days(self, days: int = 7):
        ...
        logger.info(f"Fetching stories from the last {days} days, cutoff date is {cutoff_date.date()}")
        ...
        for inx, item_id in enumerate(range(max_item_id, 0, -1)):
            ...
            if inx % 100 == 0:
                logger.info(f"Checked {inx} items so far, latest item with date {item_date.strftime('%Y-%m-%d')}")
    ...
    def crawl(self) -> None:
        ...
        logger.info(f"Retrieved {len(stories_by_list)} top, new, best, show, and ask stories")
        ...
        if self.days_back_comprehensive:
            ...
            logger.info(f"Retrieved {len(stories_by_date)} stories from the last {self.days_back} days")
        else:
            stories_by_date = []

        top_ids = list(set(list(stories_by_list) + list(stories_by_date)))[:self.N_ARTICLES]
        num_ids = len(top_ids)
        logger.info(f"Crawling {num_ids} stories")

        # Retrieve the details of each story
        for n_id, id in enumerate(top_ids):
            if n_id % 20 == 0:
                logger.info(f"Crawled {n_id} stories so far")
            try:
                self.index_story(str(id))
            except Exception as e:
                import traceback
                logger.error(f"Error crawling story {id}, error={e}, traceback={traceback.format_exc()}")
