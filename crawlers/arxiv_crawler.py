...
class ArxivCrawler(Crawler):
    ...
    def get_citations(self, arxiv_id: str) -> int:
        ...
        try:
            ...
            logger.error(f"Error parsing response from arxiv API: {e}, response={response.text}")
            return -1
    ...
    def crawl(self) -> None:
        ...
        if not validate_category(category):
            logger.error(f"Invalid arxiv category: {category}, please check the config file")
            exit(1)
        ...
        if len(papers) == 0:
            logger.warning(f"Found 0 papers for query: {query}, ignore crawl")
            return
