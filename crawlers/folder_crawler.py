...
class FileCrawlWorker(object):
    def __init__(
        self, cfg: DictConfig, indexer: Indexer, crawler: Crawler, num_per_second: int
    ):
    ...
    def process(self, file_path: str, file_name: str, metadata: dict):
        ...
        try:
            if extension in [".mp3", ".mp4"]:
                ...
                table_summarizer: TableSummarizer = TableSummarizer(
                    self.cfg, self.cfg.doc_processing.model_config.text
                )
                df_parser: DataframeParser = DataframeParser(
                    self.cfg, None, self.indexer, table_summarizer
                )
                ...
                self.indexer.index_file(
                    filename=file_path, uri=uri_to_use, metadata=metadata
                )
...
class FolderCrawler(Crawler):
    ...
    def crawl(self) -> None:
        ...
        if metadata_file:
            ...
            metadata = {
                row["filename"].strip(): row.drop("filename").to_dict()
        ...
        logger.info(
            f"indexing files in {self.cfg.folder_crawler.path} with extensions {extensions}"
        )
        files_to_process = []
        for root, _, files in os.walk(folder):
            for file in files:
                ...
                if file_extension in extensions or "*" in extensions:
                    ...
                    full_folder_path = os.path.normpath(
                        os.path.join(self.cfg.folder_crawler.path, rel_under_container)
                    )
        ...
        if ray_workers > 0:
            ...
            actors = [
                ray.remote(FileCrawlWorker).remote(self.indexer, self, num_per_second)
            ...
            _ = list(
                pool.map(
                    lambda a, u: a.process.remote(u[0], u[1], u[2]), files_to_process
                )
            )
        else:
            ...
            for inx, tup in enumerate(files_to_process):
                if inx % 100 == 0:
                    logger.info(
                        f"Crawling URL number {inx+1} out of {len(files_to_process)}"
                    )
